...
%{
    ...
    Usama Saghir
    Student no# 152105769
    email: usama.saghir@tuni.fi
%}

%   TASK 2.1 TRANSMITTER
symbol_rate = 40e6;             % Rs = 40Mhz;
M = 8;                          % Modulation order = 8
roll_off = 0.2;                 % Roll-off factor alpha
...
Tx_signal = pskmod(symbol_stream, M, pi/M,'gray');  % Applies PSK-modulation of order 'M' with phaseoffset = pi/8 and Gray mapping 
scatterplot(Tx_signal);grid on;                     % Plots the generated Tx symbols
title('8-PSK Constillation Diagram');
...
%TASK 2.2
% Using Model 10 as our complex-valued channel impulse response.
h = [ 0.8077 + 0.3767i, -0.4628 + 0.5250i, 0.5862 + 0.1055i, -0.4393 + 0.0806i, -0.2813 - 0.1445i];     % Model 10
